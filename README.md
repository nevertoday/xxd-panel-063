...
**Panel 063** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**用像素 Mask、柔和色块和负形隐喻重构照片**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
从照片中找到一个最有记忆点的大轮廓、负形或象征形态，把它作为核心 Pixel Mask；再将关键对象、动作和环境线索压缩成少量像素色块，在 Mask 内外形成嵌套、遮挡与隐喻。

```text
寻找核心轮廓或负形 → 建立一个主 Pixel Mask → 将对象与动作压缩为可辨像素块 → 通过嵌套、遮挡与错位形成隐喻 → 从原图提炼柔和有限色盘 → 以大留白控制层级
```

## 成品中最容易识别的风格特点

- 一个核心 Mask 是视觉容器，少量像素形体围绕它建立清晰层级。
- 阶梯边缘、方形模块和简化色面保留主体比例、姿态与关键特征。
- 轻微 glitch 只用于节奏：小块偏移、断裂或缺失，不演变成赛博故障风。
- 有限色盘柔和通透，少量源图特征色作为跳点，背景接近白色。
- 文字可藏进负形、沿 Mask 边缘排列或被像素局部切断，成为寓意的一部分。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-063-prompt.en.md)
...
| **[xxd-panel-063](https://github.com/nevertoday/xxd-panel-063)** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–063） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
