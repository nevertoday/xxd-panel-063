...
# 🦁 XXD Panel 063｜ピクセル隠喩手帳

### ピクセル マスク、ソフト カラー パッチ、ネガティブな比喩を使用して写真を再構築する
...
写真にデジタル感や抽象性を加えようとすると、画面いっぱいのピクセル、ネオンの乱れ、装飾的な glitch に流れがちです。効果は強くても、主体や比喩はノイズに飲み込まれてしまいます。また、写真を四角いマスクに収めただけで、空間そのものは再構成されていないケースも少なくありません。

**Panel 063** は、まず原写真にある核となる輪郭やネガティブシェイプを見つけ、それを主役の Pixel Mask に変えます。次に重要な物、動作、環境を、その境界の内外へ入れ子状に組み込みます。わずかな位置ずれはリズムだけを担い、柔らかな限定色と大きな余白が比喩を見える形にします。

### このような場面に適しています

- **デジタル感は欲しいが、サイバー系の故障ポスターにはしたくない：** ピクセル表現をノイズによる主張ではなく、主体と比喩のために使いたい。
- **抽象的でも読み取れる画面にしたい：** 一つの核となる Mask で、主体らしさ、動作、環境、ネガティブシェイプの関係を整理したい。
- **抑制の利いた実験的レイアウトが好き：** 階段状の輪郭、部分的なずれ、断裂はリズムだけに使い、柔らかな色調と白に近い背景で呼吸を保ちたい。
...
> XXD Panel 063 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
**キーワード:** ピクセル マスク · カラー ブロック コラージュ · ネガティブ ネスティング · わずかなグリッチ · 広い領域の空白

Panel 063 は、主要な主題、アクション、および環境の手がかりを少数の認識可能なピクセル形状に圧縮するためのビジュアル コンテナとしてコア マスクを使用します。映像をサイバーグリッチスタイルに押し込むのではなく、わずかなずれや途切れがリズムを​​生み出します。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 063 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | ピクセル マスク、ソフト カラー パッチ、ネガティブな比喩を使用して写真を再構築する |
| **ひと目で分かる特徴** | ピクセル マスク · カラー ブロック コラージュ · ネガティブ ネスティング · わずかなグリッチ · 広い領域の空白 |
...
ソースの最も記憶に残る大きな輪郭、ネガティブな形状、または象徴的な形状を見つけて、それを中心のピクセル マスクに変換します。主要なオブジェクト、アクション、および環境に関する手がかりは、マスク全体でネスト、オクルージョン、およびエコーするいくつかのピクセル ブロックに圧縮されます。

```text
コアの輪郭またはネガティブシェイプを見つける → 1 つのプライマリピクセルマスクを確立する → オブジェクトとアクションを認識可能なピクセルブロックに圧縮する → ネスティング、オクルージョン、ディスプレイスメントを通じてメタファーを作成する → ソースからソフトに制限されたパレットを派生する → 十分なネガティブスペースを持つ階層を制御する
```

## 完成作品を見分けるポイント

- 中央の 1 つのマスクは視覚的なコンテナとして機能し、いくつかのピクセル フォームがその周りに明確な階層を作成します。
- 階段状のエッジ、正方形のモジュール、および単純化されたカラー プレーンにより、被写体のプロポーション、姿勢、および識別特徴が維持されます。
- グリッチは、小さな変化、中断、またはブロックの欠落など、抑制されたリズムのままであり、決してサイバー障害の美学ではありません。
- 限られたパレットは柔らかく半透明のままで、白に近い地にいくつかのソースカラーがアクセントとして使用されます。
- テキストはネガティブスペースに隠れたり、マスクのエッジに続いたり、ピクセルによって局所的に中断されたりするため、メタファーに参加することがあります。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-063-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 063 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-063 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-063 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-063 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-063 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 063 を適用し、共通設定は一度だけ解決
/xxd-panel-063 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-063`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-063-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-063-prompt.zh-CN.md)
...
| **[xxd-panel-063](https://github.com/nevertoday/xxd-panel-063)** | 黒い主形 · 巨大な余白 · 網点の消散 · 禅的思考 · 思考の断片 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–063） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
