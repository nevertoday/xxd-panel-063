...
# 🦁 XXD Panel 063｜픽셀 은유 저널

### 픽셀 마스크, 부드러운 색상 패치 및 부정적인 은유를 사용하여 사진을 재구성합니다
...
사진에 디지털 감각과 추상적 의미를 더하려다 보면 화면을 가득 채운 픽셀, 네온 글리치, 장식적인 오류 효과로 흐르기 쉽습니다. 효과는 강하지만 주제와 은유는 노이즈에 삼켜집니다. 또 흔한 결과는 사진을 사각형 마스크에 넣기만 하고 공간을 실제로 재구성하지 않는 것입니다.

**Panel 063**은 먼저 원본 사진에서 핵심 윤곽이나 네거티브 스페이스를 찾아 주 Pixel Mask로 만들고, 중요한 대상과 동작, 환경을 그 경계 안팎에 겹쳐 배치합니다. 가벼운 어긋남은 리듬만 담당하고, 부드러운 제한 색상과 넓은 여백이 은유를 또렷하게 드러냅니다.

### 이런 상황에 적합합니다

- **디지털 감각은 필요하지만 사이버 글리치 포스터는 원하지 않을 때:** 픽셀 언어가 주제와 은유를 돕고, 노이즈로 존재감을 만들지 않습니다.
- **추상적인 화면도 읽을 수 있어야 할 때:** 하나의 핵심 Mask가 정체성, 동작, 환경, 네거티브 스페이스의 관계를 조직합니다.
- **절제된 실험적 레이아웃을 좋아할 때:** 계단형 가장자리, 부분적인 어긋남과 단절은 리듬만 만들고, 부드러운 색상과 거의 흰 배경은 호흡을 유지합니다.
...
> XXD Panel 063으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
**키워드:** 픽셀 마스크 · 컬러 블록 콜라주 · 네거티브 중첩 · 약간의 결함 · 넓은 공백 영역

Panel 063는 핵심 마스크를 시각적 컨테이너로 사용하여 주요 주제, 행동 및 환경 단서를 인식 가능한 소수의 픽셀 모양으로 압축합니다. 그림을 사이버 글리치 스타일로 밀어넣기보다는 약간의 어긋남과 끊김이 리듬을 담당합니다.

<!-- xxd-panel-benefit:start -->
## 빠른 판단: XXD Panel 063이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 픽셀 마스크, 부드러운 색상 패치 및 부정적인 은유를 사용하여 사진을 재구성합니다 |
| **한눈에 알아볼 특징** | 픽셀 마스크 · 컬러 블록 콜라주 · 네거티브 중첩 · 약간의 결함 · 넓은 공백 영역 |
...
소스에서 가장 기억에 남는 큰 윤곽선, 네거티브 모양 또는 상징적 형태를 찾아 중앙 픽셀 마스크로 변환합니다. 주요 개체, 동작 및 환경 단서는 해당 마스크 전체에 중첩되고, 가려지고, 에코되는 몇 개의 픽셀 블록으로 압축됩니다.

```text
핵심 윤곽 또는 네거티브 모양 찾기 → 하나의 기본 픽셀 마스크 설정 → 개체와 동작을 인식 가능한 픽셀 블록으로 압축 → 중첩, 폐색 및 변위를 통해 은유 만들기 → 소스에서 소프트 제한 팔레트 파생 → 넉넉한 네거티브 공간으로 계층 제어
```

## 완성 스타일을 알아보는 특징

- 하나의 중앙 마스크는 주변에 명확한 계층 구조를 만드는 몇 개의 픽셀 형태와 함께 시각적 컨테이너 역할을 합니다.
- 계단식 가장자리, 정사각형 모듈 및 단순화된 색상 평면은 피사체의 비율, 자세 및 식별 기능을 보존합니다.
- 글리치는 제한된 리듬(작은 교대, 중단 또는 누락된 블록)으로 유지되며 결코 사이버 장애 미학이 아닙니다.
- 제한된 팔레트는 부드럽고 반투명한 상태를 유지하며 흰색에 가까운 바탕에 몇 가지 소스 색상을 강조합니다.
- 텍스트는 네거티브 공간에 숨겨지거나, 마스크 가장자리를 따르거나, 로컬로 픽셀로 중단되어 비유에 참여할 수 있습니다.

미적 제약과 금지 항목은 [중국어 원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 런타임 어댑터는 이번 전달 변수만 처리합니다. [Skill 워크플로](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-063-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 063의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-063 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-063 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-063 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-063 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-063 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-063 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 063 적용, 공통 설정은 한 번만 확정
/xxd-panel-063 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-063`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-063-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-063-prompt.zh-CN.md)
...
| **[xxd-panel-063](https://github.com/nevertoday/xxd-panel-063)** | 검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–063) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
